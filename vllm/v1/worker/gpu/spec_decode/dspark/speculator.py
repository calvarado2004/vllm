...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def load_draft_model(
        ...
        if self.confidence_threshold > 0.0 and not model.has_confidence_head:
            raise ValueError(
                "DSpark confidence scheduling requires a checkpoint with a "
                "confidence head"
            )
        return model
    ...
    def _sample_sequential(self, num_reqs: int, head_hidden: torch.Tensor) -> None:
        ...
        confidence_markov_embeds: list[torch.Tensor] | None = (
            [] if self.draft_token_confidence_probs is not None else None
        )
        ...
        for i in range(n_spec):
            ...
            if confidence_markov_embeds is not None:
