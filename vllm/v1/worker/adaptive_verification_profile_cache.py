...
def _model_identity(runner: "GPUModelRunner") -> dict[str, Any]:
    ...
    revision = model_config.revision or resolved_revision
    if not revision:
        raise ValueError(
            "remote models require a revision or resolved commit hash for "
...
def _module_backend_state(module: torch.nn.Module | None) -> list[str]:
    ...
    for child in module.modules():
        ...
        if selected:
            selected["module"] = f"{type(child).__module__}.{type(child).__qualname__}"
