...
def test_sort_batch_req_ids_spec_decode():
    ...
    assert sort_batch_req_ids(num_tokens_per_req, {}, 2) == [
        "d1",
        "d2",
        "tail",
        "p1",
...
def test_verification_requests_lead_non_speculative_decodes():
    num_tokens_per_req = {"plain": 1, "verify_a": 4, "verify_b": 2}
    draft_tokens = {"verify_a": [1, 2, 3], "verify_b": [4]}
    assert sort_batch_req_ids(num_tokens_per_req, draft_tokens, 1) == [
        "verify_b",
        "verify_a",
        "plain",
    ]
